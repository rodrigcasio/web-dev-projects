// These are global variables used in the function below
// The file is entirely loaded before the function is called
// Therefore, these function can be declared also at the end of the file.
// for bext practices, I added them at the beginning of the file 
// to avoid confusion
...
// fully explained code for javaScript begginer in 10 points:
// 1. The code creates a canvas for drawing and initializes global variables for dimensions and context.
// 2. It defines a Vector class to represent points in 2D space with x, y, and z coordinates.
// 3. A PointCollection class manages multiple points, allowing mouse interaction and updating their positions.
// 4. Each Point has properties like current position, original position, size, and color, and methods to update and draw itself.
// 5. The code includes functions to create colors, convert phrases to hexadecimal, and handle canvas resizing and mouse movements.
// 6. It initializes event listeners for mouse movements and touch events to update the position of points based on user interaction.
// 7. The drawText function converts a name into points based on character data, applying colors and positioning them on the canvas.
// 8. The bounceBubbles function repeatedly draws and updates the points, creating an animated effect.
// 9. The shake function clears the canvas and draws points with a shaking effect when the mouse is close to them.
// 10. The code is designed to be responsive, adjusting the canvas size and point positions based on the window dimensions and user interactions.
// This code is used to create a cursor that interacts with letters on the screen
// and creates a bouncing bubble effect with customizable colors and shapes.
// It is suitable for creating interactive visualizations or animations in web applications.
// The code is structured to be modular, allowing for easy customization of colors, shapes, and behaviors.
// The main function to start the animation is drawText, which initializes the points based on a given name.
